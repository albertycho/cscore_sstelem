...
#include "/nethome/kshan9/scratch/src/sst-elements/src/sst/elements/cscore_sstelem/inc/trace_instruction.h"
#include "pin.H"
...
void WriteRegToSet(unsigned char* begin, unsigned char* end, UINT32 reg)
{
  auto set_end = std::find(begin, end, 0);
  auto found = std::find(begin, set_end, static_cast<unsigned char>(reg));
  if (found != set_end) {
    *found = static_cast<unsigned char>(reg);
  } else if (set_end != end) {
    *set_end = static_cast<unsigned char>(reg);
  }
}

void WriteMemToSet(unsigned long long* begin, unsigned long long* end, ADDRINT addr)
{
  auto set_end = std::find(begin, end, 0ULL);
  unsigned long long val = static_cast<unsigned long long>(addr);
  auto found = std::find(begin, set_end, val);
  if (found != set_end) {
    *found = val;
  } else if (set_end != end) {
    *set_end = val;
...
VOID Instruction(INS ins, VOID* v)
{
  ...
  for (UINT32 i = 0; i < readRegCount; i++) {
    ...
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)WriteRegToSet, IARG_PTR, curr_instr.source_registers, IARG_PTR,
  ...
  for (UINT32 i = 0; i < writeRegCount; i++) {
    ...
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)WriteRegToSet, IARG_PTR, curr_instr.destination_registers, IARG_PTR,
  ...
  for (UINT32 memOp = 0; memOp < memOperands; memOp++) {
    if (INS_MemoryOperandIsRead(ins, memOp))
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)WriteMemToSet, IARG_PTR, curr_instr.source_memory, IARG_PTR,
                     curr_instr.source_memory + NUM_INSTR_SOURCES, IARG_MEMORYOP_EA, memOp, IARG_END);
    if (INS_MemoryOperandIsWritten(ins, memOp))
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)WriteMemToSet, IARG_PTR, curr_instr.destination_memory, IARG_PTR,

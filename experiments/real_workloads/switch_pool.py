...
for node_id in range(NUM_NODES):
    ...
    link_node_to_switch = sst.Link(f"s{node_id}_to_switch0")
    link_node_to_switch.connect(
        (sock, "port_handler_cxl", "1ns"),
        (switch, f"port_handler_nodes{node_id}", "1ns"),
    )

link_switch_to_pool = sst.Link("switch0_to_pool0")
link_switch_to_pool.connect(
    (switch, "port_handler_pools0", "1ns"),

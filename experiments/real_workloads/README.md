This directory runs minimal CoaXiaL-style experiments in the existing simulator: one ChampSim-trace-driven node connected to pooled memory, with the full traced address space mapped to the pool.
...
All three topologies default to `WARMUP_INSTS=5000000`, `SIM_INSTS=20000000`, and a CPU heartbeat every `100000` cycles. `all_to_pool_node0.csv` maps the lower canonical user virtual-address range to pool node `100`, so all traced pages are treated as remote pooled memory. The switch-based topologies reuse that same mapping; the switch selects among backend pools.

...
OUT_PNG_PROJECTED = LOG_DIR / "bw_latency_curves_projected_total_bw.png"
...
STAT_NODE_HOST_TO_RE = re.compile(r"^stat\.node\.(\d+)\.bw\.host_to_switch_gbps\s*=\s*([0-9.eE+-]+)\s*$")
STAT_NODE_TO_HOST_RE = re.compile(r"^stat\.node\.(\d+)\.bw\.switch_to_host_gbps\s*=\s*([0-9.eE+-]+)\s*$")
STAT_NODE_TOTAL_RE = re.compile(r"^stat\.node\.(\d+)\.bw\.host_link_total_gbps\s*=\s*([0-9.eE+-]+)\s*$")
...
def parse_node_stats(out_path: Path):
    ...
    with out_path.open() as f:
        for raw in f:
            ...
            if mu:
                ...
                continue
            mn_h2s = STAT_NODE_HOST_TO_RE.match(line)
            if mn_h2s:
                node = int(mn_h2s.group(1))
                node_bw[node]["host_to_switch_gbps"] = float(mn_h2s.group(2))
                continue
            mn_s2h = STAT_NODE_TO_HOST_RE.match(line)
            if mn_s2h:
                node = int(mn_s2h.group(1))
                node_bw[node]["switch_to_host_gbps"] = float(mn_s2h.group(2))
                continue
            mn_tot = STAT_NODE_TOTAL_RE.match(line)
            if mn_tot:
                node = int(mn_tot.group(1))
                node_bw[node]["host_link_total_gbps"] = float(mn_tot.group(2))
                continue
    ...
    return node_miss, node_lat, node_bw, pool_req_link_util, sim_time_s, switch_bw
...
def collect_rows(log_dir: Path, bw_table):
    ...
    for out_path in sorted(log_dir.glob("run_*.out")):
        ...
        node_miss, node_lat, node_bw, pool_req_link_util, sim_time_s, switch_bw = parse_node_stats(out_path)
        ...
        if not node_bw:
            continue
        observed_h2s = sum(v["host_to_switch_gbps"] for v in node_bw.values())
        observed_s2h = sum(v["switch_to_host_gbps"] for v in node_bw.values())
        observed_total = sum(
            (v["host_link_total_gbps"] if v["host_link_total_gbps"] > 0.0
             else v["host_to_switch_gbps"] + v["switch_to_host_gbps"])
            for v in node_bw.values()
        )
        ...
        switch_stat_h2s = switch_bw["host_to_switch_gbps"]
        switch_stat_s2h = switch_bw["switch_to_host_gbps"]
        switch_stat_total = switch_bw["host_link_total_gbps"]
        switch_stat_bottleneck = max(switch_stat_h2s, switch_stat_s2h)
        switch_stat_balance_ratio = 0.0
        if switch_stat_bottleneck > 0.0:
            switch_stat_balance_ratio = min(switch_stat_h2s, switch_stat_s2h) / switch_stat_bottleneck
        ...
        rows.append(
            {
                ...
                "observed_host_to_switch_gbps": observed_h2s,
                "observed_switch_to_host_gbps": observed_s2h,
                "observed_host_link_total_gbps": observed_total,
                "observed_bottleneck_gbps": observed_bottleneck,
                "observed_balance_ratio": observed_balance_ratio,
                "switch_stat_host_to_switch_gbps": switch_stat_h2s,
                "switch_stat_switch_to_host_gbps": switch_stat_s2h,
                "switch_stat_host_link_total_gbps": switch_stat_total,
                "switch_stat_bottleneck_gbps": switch_stat_bottleneck,
                "switch_stat_balance_ratio": switch_stat_balance_ratio,
...
def plot(
    ...
    msg1 = _plot_curves(
        rows,
        out_png_main,
        x_key="observed_host_link_total_gbps",
        x_label="Observed aggregate node host-link BW (GB/s)",
        title_suffix="CXL Latency vs Observed Aggregate Node Host-Link Bandwidth",
    )
    msg2 = _plot_curves(
        rows,
        out_png_projected,
    ...
    msg4 = _plot_curves(
        rows,
        out_png_switch_obs,
        x_key="switch_stat_host_link_total_gbps",
    ...
    msg5 = _plot_curves(
        ...
        x_key="switch_stat_bottleneck_gbps",
...
def plot_iso_total_balance(rows, out_png: Path, bin_width_gbps: float = 0.25):
    ...
    for ax, mode in zip(axes, modes):
        ...
        for r in mode_rows:
            total = float(r["observed_host_link_total_gbps"])
            if total <= 0.0:
                continue
        ...
        for b in sorted(buckets.keys()):
            ...
            for r in bucket:
                h2s = float(r["observed_host_to_switch_gbps"])
...
def main():
    ...
    msg1, msg2, msg3, msg4, msg5, msg6, msg7 = plot(
        rows,
        OUT_PNG,
        OUT_PNG_PROJECTED,
        OUT_PNG_ACTUAL,
    ...
    print(
        _plot_mode_panels(
            rows,
            OUT_PNG_LOAD_SWEEP,
            x_key="observed_bottleneck_gbps",
            curve_key="mem_pct",
            x_label="Observed aggregate node directional bottleneck BW (GB/s)",
            title_suffix="CXL Latency vs Node Directional Bottleneck BW (curves by Mem Ratio)",
            curve_label="M",
        )
    )
    print(

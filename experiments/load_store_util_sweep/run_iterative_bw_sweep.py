...
def parse_run_output(out_path: Path) -> dict:
    ...
    with out_path.open("r", errors="ignore") as f:
        for raw in f:
            ...
            if mn_h2s:
                ...
                node_bw.setdefault(node, {})["host_to_switch_gbps"] = float(mn_h2s.group(2))
                continue
            mn_s2h = STAT_NODE_TO_HOST_RE.match(line)
            if mn_s2h:
                node = int(mn_s2h.group(1))
                node_bw.setdefault(node, {})["switch_to_host_gbps"] = float(mn_s2h.group(2))
                continue
            mn_tot = STAT_NODE_TOTAL_RE.match(line)
            if mn_tot:
                node = int(mn_tot.group(1))
                node_bw.setdefault(node, {})["host_link_total_gbps"] = float(mn_tot.group(2))
                continue
    ...
    if not node_bw:
        raise ValueError(f"Missing node bandwidth stats in {out_path}")
    observed_h2s = sum(v.get("host_to_switch_gbps", 0.0) for v in node_bw.values())
    observed_s2h = sum(v.get("switch_to_host_gbps", 0.0) for v in node_bw.values())
    observed_total = sum(
        v.get("host_link_total_gbps", v.get("host_to_switch_gbps", 0.0) + v.get("switch_to_host_gbps", 0.0))
    ...
    return {
        ...
        "switch_stat_host_to_switch_gbps": host_to_switch,
        "switch_stat_switch_to_host_gbps": switch_to_host,
        "switch_stat_host_link_total_gbps": host_link_total,
...
def run_case(mode: str, load_pct: int) -> list[dict]:
    ...
    while target <= TARGET_MAX_GBPS + 1e-9:
        ...
        obs_bw = stats["observed_host_link_total_gbps"]
...
def write_summary(rows: list[dict], out_csv: Path) -> None:
    ...
    keys = [
        ...
        "switch_stat_host_to_switch_gbps",
        "switch_stat_switch_to_host_gbps",
        "switch_stat_host_link_total_gbps",
        "observed_host_to_switch_gbps",
        "observed_switch_to_host_gbps",
        "observed_host_link_total_gbps",

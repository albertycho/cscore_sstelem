...
void CACHE::finish_translation(const response_type& packet)
{
  ...
  auto mark_translated = [p_page = champsim::page_number{packet.data}, this](auto& entry) {
    ...
    if (address_map) {
      auto map_entry = address_map->lookup(static_cast<uint32_t>(node_id), entry.v_address.template to<uint64_t>());
      if (map_entry && map_entry->type == SST::csimCore::AddressType::Pool) {
        auto offset = entry.v_address.template to<uint64_t>() - map_entry->start;
